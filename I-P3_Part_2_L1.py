...
file_prefix = 'L1_' + 'MADPROPS_APR_21_2021_FINAL_'
...
for gene in chosen_genes:
    ...
    for i in range(len(translated_reference_sequence_concatenate_STR)):
        ...
        if ((identity_test_codon_1 == 1).any()) & ((identity_test_codon_2 == 1).any()):
            if (identity_test_codon_1[identity_test_codon_1 == 1].index[0] == 'T') & (identity_test_codon_2[identity_test_codon_2 == 1].index[0] == 'T') & (amino_acid in 'L'):
                j = i*3+2
                ...
                if ('T' not in nucleotide_possibilities_at_degenerate_string) & ('C' not in nucleotide_possibilities_at_degenerate_string):
                    degenerate_count += 1
