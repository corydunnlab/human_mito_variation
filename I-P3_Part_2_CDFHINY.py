...
file_prefix = 'CDFHINY_' + 'MADPROPS_APR_21_2021_FINAL_'
...
for gene in chosen_genes:
    ...
    for i in range(len(translated_reference_sequence_concatenate_STR)):
        ...
        if ((identity_test_codon_1 == 1).any()) & ((identity_test_codon_2 == 1).any()) & (amino_acid in 'CDFHINY'):
            j = i*3+2
            ...
            if ('G' not in nucleotide_possibilities_at_degenerate_string) & ('A' not in nucleotide_possibilities_at_degenerate_string):
                degenerate_count += 1
